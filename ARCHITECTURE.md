...
| **추천 매매가(trade_median) 기준 기간** | `src/analysis/recommend.py::_trade_agg()`/`_recent_window()` (최근 `trade_months`개월 median, 없으면 전체기간 fallback) | `recommend_gap_investment/rental_yield/buy_outright/investment_focus()` 4개 전략 전부, `src/ui/shared/cache.py` 4개 `_cached_*` 래퍼, `src/ui/pages/invest.py` "현재 매매가 기준 기간" 슬라이더 | **2026-09-20 신설**: `recommend_buy_outright`·`recommend_investment_focus`가 분석기간(months, 기본 12·24개월) 전체 median을 그대로 매매가로 써서, 최근 급등한 단지의 추천가가 실제 호가보다 훨씬 낮게 표시되던 문제(갭투자·임대수익은 이미 `trade_months=3` 최근창을 쓰고 있어 두 전략만 어긋나 있었음)를 수정. 4개 전략 모두 `_trade_agg()` 공용 사용으로 통일. 분석기간(months) 슬라이더를 늘려도 매매가 기준일이 같이 밀리지 않도록 별도 슬라이더로 분리 — 두 슬라이더 의미가 바뀌면 `invest.py`의 help 텍스트도 같이 고칠 것. 같은 날 기본값을 `trade_months=3` → 2개월 → **1개월**로 재조정(4개 함수 + `cache.py` 4개 래퍼 + `invest_recommend.py`/`invest_compare.py`의 fallback 기본값 전부 동일하게 맞춤). 1개월은 국토부 실거래 신고기한(계약일로부터 30일 이내)보다 짧아 그 달 거래가 아직 다 안 들어와 있을 수 있는데, `_trade_agg`/`_recent_window`의 fallback(해당 창에 거래 0건이면 전체기간 median 사용)이 이미 이 상황을 처리하므로 별도 방어 로직 추가는 불필요. `invest.py` 검색폼의 준공연도 기본값도 같이 조정: 하한 `이번해-10년` → **2000년**(분양권 포함 더 넓게 탐색). 전용면적 기본값은 80~110㎡(전용면적을 그대로 3.3㎡/평으로 환산한 값)이 실제 시장 '평형' 표기(공급면적 기준, 전용 84㎡=34평형·전용 59㎡=24~25평형)와 달라 24평형이 걸러지는 문제가 있어 **60~85㎡**(국민주택규모 상한 기준)로 수정 |
